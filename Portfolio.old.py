...
from MarketObjects import Order, Position
...
class Portfolio(Thread):
    ...
    def add(self,interface):
        ...
        self.strategy_positions[interface.name] = {}
        self.equity_curves[interface.name] = {}
        self.trading_activity[interface.name] = {}

        self.strategy_attributes[interface.name] = dict(interface.strategy_params)

        self.STRAT_orders[interface.name] = interface.OUT_orders
        self.STRAT_fills[interface.name] = interface.IN_fills
    ...
    def win_loss_stats(self,tag,stat_table):
        ...
        dct = {'tag':tag,'cnt':cnt,'w_avg':avg_win,'w_std':stdv_win, 'pr':profit_ratio, \
    ...
    def equ_curve_stats(self,tag,curve_table):
        ...
        return {'tag':tag, 'mtm_pnl':mtm_pnl, 'max_equ':max_equ,'max_dd':max_dd,'avg_dd':avg_dd}
    ...
    def blend_curve(self,curve1,curve2,column):
        ...
        if curve1 is not None and 'timestamp' in curve1.columns.tolist():    
    ...
    def stats(self):
        ...
        for strat, curves in self.equity_curves.iteritems():
            ...
            for sym in curve_keys:
                ...
                curve_df.drop_duplicates(subset=['timestamp'],take_last=True,inplace=True)
            ...
            with self.lock:
                ...
                try:
                    ...
                    if fill.timestamp <= self.current_timestamp:
                        ...
                        self.log.info("push fill: %s" % fill)
                        self.STRAT_fills[order.owner].put(fill)
                ...
                for strat in self.STRAT_orders.keys():
                    try:
                        ...
                        self.log.info("send order: %s" % order)
                        self.OUT_orders.put(order)
    ...
    def on_data(self,market_data):
        ...
        if market_data:
            ...
            with self.lock:
                self.log.info("__ON_DATA: %s" % market_data)

                ## deplete fill queue first
                report_push = False
                while self.IN_fills:
                    if not report_push:
                        self.log.info('pushing fills before mark_to_market() call')
                        report_push = False
...
class StorageToExcel(object):
    ...
    def to_excel(self,filename=None):
        ...
        try:
            ...
            print "Unable to write output file: '%s' for optimizer" % filename
            print type(e)
            success = False
