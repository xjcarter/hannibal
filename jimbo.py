...
it takes a trading blotter DataFrame as input, and a needs a data_source directory 
to do mark-to-market calculations
...
self.storage = dict of pnl curve dataframes indexed by symbol
...
class ScoreCard(object):
	...
	def bind_curve(self,symbol,df):
		...
		try:
			...
			g['mark_pnl'] = g['net_position']*(g['avg_price']-g['close'])
	 
			g['realized_pnl'] = g['trade_pnl'].cumsum()
			g['tot_pnl'] = g['mark_pnl'] + g['realized_pnl']

			g = g[['date','symbol','trade','price','net_position','avg_price','trade_pnl','mark_pnl','realized_pnl','tot_pnl']]
